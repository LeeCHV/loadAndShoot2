...
void m3508::CAN_Send(int16_t current1, int16_t current2, int16_t current3,
                     int16_t current4) // CAN 发送 一标准帧数据
{
...
void m3508::c620_read() {
  ...
  uint16_t actual_local_position =
      (uint16_t)(receive_msg.data[0] << 8) | receive_msg.data[1];
  int16_t actual_velocity =
      (int16_t)(receive_msg.data[2] << 8) | receive_msg.data[3];
  int16_t actual_current =
      (int16_t)(receive_msg.data[4] << 8) | receive_msg.data[5];
  ...
  global_pos[motor_index] = round_cnt[motor_index] * 8192 +
                            read_position[motor_index] - start_pos[motor_index];
  // Map to degree, 0-360
...
void m3508::c620_write() // Execution frequency 100Hz
{
  ...
  for (int motor_index = 0; motor_index < motor_num; motor_index++) {
    if (loop_mode[motor_index] == m3508_vel_loop) {
      // velocity loop
      v_pid[motor_index].P =
          required_velocity[motor_index] - read_velocity[motor_index];
      ...
      motor_out[motor_index] =
          (int16_t)floor((v_pid[motor_index].P * v_pid[motor_index].kP +
      ...
      // position loop
      p_pid[motor_index].P =
          required_position[motor_index] - global_angle[motor_index];
      p_pid[motor_index].P = constrain(p_pid[motor_index].P, -4096, 4096);
      p_pid[motor_index].I += p_pid[motor_index].P * dt;
      p_pid[motor_index].D =
          (p_pid[motor_index].P - p_pid[motor_index].prev_err) / dt;
      ...
      required_velocity[motor_index] =
          (int16_t)floor(p_pid[motor_index].kP * p_pid[motor_index].P +
                         p_pid[motor_index].kI * p_pid[motor_index].I +
                         p_pid[motor_index].kD * p_pid[motor_index].D);
      // printf("%d\r\n",required_velocity[motor_index]);
    ...
    motor_out[motor_index] = constrain(motor_out[motor_index], -7000, 7000); //3500/700 = 5A
  }
  for (int motor_index = 0; motor_index < motor_num; motor_index++) {
...
int m3508::get_velocity(int motor_index) 
{
  return required_velocity[motor_index];
}

float m3508::get_position(int motor_index) 
{
  return required_position[motor_index];
}
